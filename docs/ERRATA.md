...
Pendências detectadas na auditoria de documentação (`docs-audit-meta-platform.md`)
...
| 3 | Divergência de nomenclatura do artefato: build gera `...-0.0.19-alpha-linux-x64`, mas docs/URLs de release usam `...-0.0.19-preview-linux-x64`. | build/release | Médio: link de download pode não bater com o artefato gerado pelo script `build`. | `Meta-Platform/meta-platform-setup-wizard-command-line/package.json` (script `build`) e URLs de release | Padronizar `alpha`/`preview` entre o script `build` e os assets de release. |
